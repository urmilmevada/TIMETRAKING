...
class ManagerPage(ListView):
    ...
    def get(self,request,*args,**kwargs):
        project = Timetracker.objects.all().values()
        team = Project_Team.objects.all().values()
        module = Project_Module.objects.all().values()
        task = Project_Task.objects.all().values()
        completedproject = Timetracker.objects.filter(status="Completed")
        pendingproject = Timetracker.objects.filter(status="Pending")
        chs = Timetracker.objects.annotate(month=ExtractMonth('project_start_date')) \
                                  .values('month') \
                                  .annotate(total_projects=Count('id')) \
                                  .order_by('month')
        
        completed_projects = Timetracker.objects.filter(status="Completed")
        pending_projects = Timetracker.objects.filter(status="Pending")
        cancelled_projects = Timetracker.objects.filter(status="Cancelled")
        total_projects = Timetracker.objects.aggregate(
